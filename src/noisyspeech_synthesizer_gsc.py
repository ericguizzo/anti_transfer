...
input_noisyspeech_repo = ('preprocessing', 'input_noisyspeech_repo')
#input_gsc_folder = ('preprocessing', 'balanced_gsc_folder')
#output_dir = cfg.get('preprocessing', 'input_gsc_noisy_folder')
#output_dir = '../../shared_datasets/speechCmd/noisy_balanced'
augmented_gsc_folder = ('preprocessing', 'augmented_gsc_folder')
input_gsc_folder = os.path.join(augmented_gsc_folder, 'balanced')
output_dir = os.path.join(augmented_gsc_folder, 'noisy_balanced')
...
def main(cfg):
    ...
    '''
    clean_dir = input_gsc_folder
    noise_dir = os.path.join(input_noisyspeech_repo, 'noise_train')
    ...
    #cleanfilenames = cleanfilenames[:10]
    for i in range(len(cleanfilenames)):
        idx_s = i
        clean, fs = audioread(cleanfilenames[idx_s])
        old_filename = cleanfilenames[idx_s].split('/')[-1].split('.')[0]
        '''
        ...
        print (cleanfilenames[idx_s],noisefilenames[idx_n])
        ...
        rand_snd = random.choice(SNR)
        clean_snr, noise_snr, noisy_snr = snr_mixer(clean=clean, noise=noise, snr=rand_snd)
        #noisyfilename = 'noisy'+str(filecounter)+'_SNRdb_'+str(SNR[i])+'_clnsp'+str(filecounter)+'.wav'

        noisyfilename = 'noisy'+str(filecounter)+'_SNRdb_'+str(rand_snd)+'_'+old_filename+'.wav'

        cleanfilename = 'clnsp'+str(filecounter)+'.wav'
        noisefilename = 'noisy'+str(filecounter)+'_SNRdb_'+str(rand_snd)+'.wav'
...
if __name__=="__main__":
    ...
    # Configurations: read noisyspeech_synthesizer_gsc.cfg
    parser.add_argument("--cfg", default = "noisyspeech_synthesizer_gsc.cfg", help = "Read noisyspeech_synthesizer_gsc.cfg for all the details")
