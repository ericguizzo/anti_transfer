...
def load_feature_extractor(gpu_id, model_url, pretraining_classes_load):
...
def feature_loss(input, current_model, pretrained_model, beta=1., layer=28,
                aggregation='none', distance='mse_sigmoid', gpu_ID=0):
    ...
    if not isinstance(layer, list):
        ...
        layer_init = layer
        loss_aggregated = torch.tensor(0).float().to(dev)
    del layer
    n_layers = len(layer_init)

    for layer in layer_init:
    ...
    if n_layers == 1:
        ...
        loss_aggregated /- n_layers
        output_loss = loss_aggregated
