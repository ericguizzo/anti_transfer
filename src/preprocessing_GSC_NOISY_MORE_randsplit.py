...
INPUT_GSC_FOLDER = cfg.get('preprocessing', 'input_gsc_noisy_MORE_folder')
...
for sound in sounds_list:
    ...
    label = sound.split('/')[-1].split('.')[-2].split('_')[-1]
...
def main():
    ...
    if AUGMENTATION:
        predictors_save_path = os.path.join(OUTPUT_FOLDER, 'gsc_noisy_more_randsplit' + appendix + '_aug' + str(NUM_AUG_SAMPLES) + '_predictors.npy')
        target_save_path = os.path.join(OUTPUT_FOLDER, 'gsc_noisy_more_randsplit' + appendix + '_aug' + str(NUM_AUG_SAMPLES) + '_target.npy')
    else:
        predictors_save_path = os.path.join(OUTPUT_FOLDER, 'gsc_noisy_more_randsplit' + appendix + '_predictors.npy')
        target_save_path = os.path.join(OUTPUT_FOLDER, 'gsc_noisy_more_randsplit' + appendix + '_target.npy')
