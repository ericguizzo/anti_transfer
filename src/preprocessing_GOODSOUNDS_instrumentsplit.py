...
def main():
    ...
    if AUGMENTATION:
        predictors_save_path = os.path.join(OUTPUT_FOLDER, 'goodsounds_instrumentsplit' + appendix + '_aug' + str(NUM_AUG_SAMPLES) + '_predictors.npy')
        target_save_path = os.path.join(OUTPUT_FOLDER, 'goodsounds_instrumentsplit' + appendix + '_aug' + str(NUM_AUG_SAMPLES) + '_target.npy')
    else:
        predictors_save_path = os.path.join(OUTPUT_FOLDER, 'goodsounds_instrumentsplit' + appendix + '_predictors.npy')
        target_save_path = os.path.join(OUTPUT_FOLDER, 'goodsounds_instrumentsplit' + appendix + '_target.npy')
    ...
    for i in range(num_foldables):
        ...
        curr_list = filter_instruments_GOODSOUNDS(sounds_list, i)
        print (instruments[i],len(curr_list))
        #curr_list = curr_list[:10]
        #preprocess all sounds of the current actor
        #args:1. listof soundpaths of current actor, 2. max file length, 3. function to extract lable from filepath

        curr_predictors, curr_target = pre.preprocess_foldable_item(curr_list, max_file_length, get_lable_GOODSOUNDS, True)
