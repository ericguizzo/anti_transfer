...
output_dir = cfg.get('preprocessing', 'generated_noisyspeech_folder')
#output_dir = '../dataset/noisy_speech'
#output_dir = '../../shared_datasets/MS-SNSD/noisy_selected'
...
def main(cfg):
    ...
    '''
    clean_dir = os.path.join(input_noisyspeech_repo, 'clean_train')
    ...
    while num_samples < total_samples:
        ...
        for i in range(np.size(SNR)):
            ...
            noise_tag = noisefilenames[idx_n].split('/')[-1].split('_')[0]

            noisyfilename = 'noisy'+str(filecounter)+'_SNRdb_'+str(SNR[i])+'_'+noise_tag+'.wav'
