...
pretraining_dataset_path = '../dataset/matrices/noisy_speech'
num_cnn_layers = cfg.getint('pretraining', 'num_cnn_layers')
preprocessing_ID = 'first'
sequence_length = 1
num_speakers_noisyspeech = 11
...
base_results = '../pretraining_vgg/noisyspeech'
...
PREDICTORS_LOAD = os.path.join(pretraining_dataset_path, 'noisyspeech_randsplit_spectrum_fast_predictors.npy')
TARGET_LOAD = os.path.join(pretraining_dataset_path, 'noisyspeech_randsplit_spectrum_fast_target.npy')
...
#load model
par = ['output_classes=' + str(num_speakers_noisyspeech)]
